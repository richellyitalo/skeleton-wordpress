...
function feed_facebook($fanpage = 'nikefutebol', $limit = 2, $offset = 0) {
    ...
    $feed = fetchUrl("https://graph.facebook.com/{$fanpage}/posts?limit={$limit}&offset={$offset}&access_token=396646610471255|rIw3iyPeaaB8cAsDmoOxdsXOPQM");
...
//        $i = 0;
    foreach($fbfeed->data as $post){
        if($post->type == 'photo'){
            $post->thumbnail = "https://graph.facebook.com/{$post->object_id}/picture?type=normal&amp;width=750&amp;height=537";
